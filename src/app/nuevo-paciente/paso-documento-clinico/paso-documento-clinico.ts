...
export class PasoDocumentoClinico implements OnInit, OnDestroy {
  ...
  private loadTiposDocumento(): void {
    ...
    this.catalogoService.getTiposDocumento()
      ...
      .subscribe({
        next: (tipos) => {
          this.tiposDocumento = tipos.map(tipo => this.mapTipoDocumentoToLocal(tipo));
  ...
  private mapTipoDocumentoToLocal(tipoBackend: any): TipoDocumento {
    ...
    return {
      ...
      popular: tiposPopulares.includes(tipoBackend.nombre)
    };
  }
  ...
  get tiposPopulares(): TipoDocumento[] {
    return this.tiposDocumento.filter(tipo => tipo.popular);
  }
